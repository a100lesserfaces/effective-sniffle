...
PREPARE_LOGGING(test_component_i)

test_component_i::test_component_i(const char *uuid, const char *label) :
    test_component_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

test_component_i::~test_component_i()
{
}

void test_component_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void test_component_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &test_component_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &test_component_i::<callback method>)
        in the constructor.
        ...
        //Add to test_component.cpp
        test_component_i::test_component_i(const char *uuid, const char *label) :
            test_component_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &test_component_i::scaleChanged);
            addPropertyListener(status, this, &test_component_i::statusChanged);
        }

        void test_component_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(test_component_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void test_component_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(test_component_i, "status changed");
        }
            
        //Add to test_component.h
...
************************************************************************************************/
int test_component_i::serviceFunction()
{
    LOG_DEBUG(test_component_i, "serviceFunction() example log message");
    
    LOG_INFO(test_component_i, m_Tester.testLibrary());

    LOG_INFO(test_component_i, m_Speedy.getMaxDiskSpeed());
